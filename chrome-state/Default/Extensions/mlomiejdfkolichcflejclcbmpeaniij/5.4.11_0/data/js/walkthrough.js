...
require([
	...
	function saveWalkthrough() {
		...
		conf.ghostrank = (typeof ghostrank_value == 'undefined' ? false : ghostrank_value);
	...
	function initNavigation() {
		var slides = $('#slider').children(),
			...
			headerTitles = [
				...
				t('walkthrough_ghostrank_header'),
		...
		function onNavigate() {
			...
			$('#nav-prev').toggle(current > 0);
			$('#skip-button').toggle(current === 0);
			$('#nav-next').toggle(current < slides.length - 1)
				// wait for ghostrank selection
				.prop('disabled', (current == 1 && typeof ghostrank_value == 'undefined'));
		...
		function next() {
			if (current + 1 >= slides.length || $('#nav-next').prop('disabled')) {
				return;
			}
	...
	function ghostrankSelected(id) {
		$('#nav-next').prop('disabled', false);
		if (id == 'ghostrank-on') {
			ghostrank_value = true;
			$('#ghostrank-on').addClass('enabled');
			$('#ghostrank-off').removeClass('enabled');
		} else {
			ghostrank_value = false;
			$('#ghostrank-on').removeClass('enabled');
			$('#ghostrank-off').addClass('enabled');
		}
	}
	...
	function loadWalkthrough(bp) {
		...
		$('button.ghostrank-button').click(function (e) {
			e.preventDefault();
			ghostrankSelected(e.target.parentElement.id);
		});
